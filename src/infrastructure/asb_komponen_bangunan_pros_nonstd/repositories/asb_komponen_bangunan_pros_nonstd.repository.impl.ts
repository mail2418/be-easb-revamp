...
export class AsbKomponenBangunanProsNonstdRepositoryImpl implements AsbKomponenBangunanProsNonstdRepository {
    ...
    async findAll(pagination: GetAsbKomponenBangunanProsNonstdListDto): Promise<{ data: AsbKomponenBangunanProsNonstd[], total: number }> {
        try {
            const qb = this.repo.createQueryBuilder('asb_komponen_bangunan_pros_nonstd')
                .leftJoinAndSelect('asb_komponen_bangunan_pros_nonstd.asbKomponenBangunanNonstd', 'asbKomponenBangunanNonstd');

            applyIlikeSearch(qb, 'asbKomponenBangunanNonstd', ['komponen'], pagination.search);
