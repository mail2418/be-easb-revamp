...
export class JalanSaluranRuangLingkupController {
    ...
    @Post() async create(@Body() dto: CreateJalanSaluranRuangLingkupDto): Promise<ResponseDto> { try { return toResponseDto(await this.service.create(dto), 'Created', HttpStatus.CREATED); } catch (e) { return toErrorResponseDto(e); } }
    @Put() async update(@Body() dto: UpdateJalanSaluranRuangLingkupDto): Promise<ResponseDto> { try { return toResponseDto(await this.service.update(dto), 'Updated'); } catch (e) { return toErrorResponseDto(e); } }
