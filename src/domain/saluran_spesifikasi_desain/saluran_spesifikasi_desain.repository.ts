...
export abstract class SaluranSpesifikasiDesainRepository {
    ...
    abstract findAll(dto: GetSaluranSpesifikasiDesainDto): Promise<{ data: SaluranSpesifikasiDesain[]; total: number }>;
    abstract deleteByUsulanSaluranId(idUsulanSaluran: number): Promise<void>;
    abstract findByUsulanSaluran(idUsulanSaluran: number, page?: number, amount?: number): Promise<[SaluranSpesifikasiDesain[], number]>;
}
