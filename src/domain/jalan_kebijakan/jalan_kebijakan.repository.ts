...
export abstract class JalanKebijakanRepository {
    ...
    abstract findByKabkotaBulanTahun(
        idKabkota: number,
        bulan: number,
        tahun: number,
    ): Promise<JalanKebijakan | null>;
    abstract findAll(dto: GetJalanKebijakanDto): Promise<{ data: JalanKebijakan[]; total: number }>;
}
