...
export class CalculateVolumeSaluranSpesifikasiDesainReviewUseCase {
    ...
    async execute(idHspk: number, lebar: number, tinggiReview: number, spasiReview: number): Promise<number> {
        ...
        else if (satuan === 'Kg') volume = 7850 * (Math.PI / 4) * Math.ceil(1000 / spasiReview) * Math.pow(tinggiReview / 1000, 2);
