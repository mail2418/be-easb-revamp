...
export class JalanKebijakanServiceImpl implements JalanKebijakanService {
    ...
    async create(dto: CreateJalanKebijakanDto): Promise<JalanKebijakan> {
        const existing = await this.repository.findByKabkotaBulanTahun(
            dto.idKabkota,
            dto.bulan,
            dto.tahun,
        );
        if (existing) {
            throw new ConflictException(
                `JalanKebijakan with kabkota ${dto.idKabkota}, bulan ${dto.bulan}, tahun ${dto.tahun} already exists`,
            );
        }
    ...
    async update(dto: UpdateJalanKebijakanDto): Promise<JalanKebijakan> {
        ...
        if (
            targetKabkota !== existing.idKabkota ||
            targetBulan !== existing.bulan ||
            targetTahun !== existing.tahun
        ) {
            const duplicate = await this.repository.findByKabkotaBulanTahun(
                targetKabkota,
                targetBulan,
                targetTahun,
            );
            if (duplicate) {
                throw new ConflictException(
                    `JalanKebijakan with kabkota ${targetKabkota}, bulan ${targetBulan}, tahun ${targetTahun} already exists`,
                );
            }
        }
