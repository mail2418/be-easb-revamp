...
export class JalanSpesifikasiDesainServiceImpl implements JalanSpesifikasiDesainService {
    ...
    async create(
        dto: CreateJalanSpesifikasiDesainDto,
        lebar?: number,
    ): Promise<JalanSpesifikasiDesain> {
        ...
        if (lebarValue === undefined) {
            ...
            if (!usulanJalan) {
                throw new NotFoundException(
                    `Usulan Jalan with id ${dto.id_usulan_jalan} not found`,
                );
            }
            if (usulanJalan.lebar == null) {
                throw new NotFoundException(
                    `Usulan Jalan with id ${dto.id_usulan_jalan} has no lebar`,
                );
            }
    ...
    ): Promise<JalanSpesifikasiDesainPaginationResultDto> {
    ...
    async getByUsulanJalan(dto: GetJalanSpesifikasiDesainByUsulanJalanDto): Promise<{
        ...
        const [data, total] = await this.jalanSpesifikasiDesainRepository.findByUsulanJalan(
            dto.idUsulanJalan,
            dto.page,
            dto.amount,
        );
