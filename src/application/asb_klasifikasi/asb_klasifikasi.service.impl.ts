...
export class AsbKlasifikasiServiceImpl implements AsbKlasifikasiService {
    ...
    async create(asbKlasifikasi: CreateAsbKlasifikasiDto): Promise<AsbKlasifikasi> {
        ...
        const existingAsbKlasifikasi = await this.asbKlasifikasiRepository.findByKlasifikasi(
            asbKlasifikasi.klasifikasi,
        );
        if (existingAsbKlasifikasi) {
            throw new ConflictException(
                `AsbKlasifikasi with klasifikasi ${asbKlasifikasi.klasifikasi} already exists`,
            );
        }
        ...
        if (!this.asbTipeBangunanRepository.findById) {
            throw new NotFoundException(
                `TipeBangunan with id ${asbKlasifikasi.id_asb_tipe_bangunan} not found`,
            );
        }
    ...
    async update(asbKlasifikasi: UpdateAsbKlasifikasiDto): Promise<AsbKlasifikasi> {
        ...
        const existingAsbKlasifikasi = await this.asbKlasifikasiRepository.findById(
            asbKlasifikasi.id,
        );
        if (!existingAsbKlasifikasi) {
        ...
        if (
            asbKlasifikasi.klasifikasi &&
            asbKlasifikasi.klasifikasi !== existingAsbKlasifikasi.klasifikasi
        ) {
            const duplicateAsbKlasifikasi = await this.asbKlasifikasiRepository.findByKlasifikasi(
                asbKlasifikasi.klasifikasi,
            );
        ...
        const updatedAsbKlasifikasi = await this.asbKlasifikasiRepository.update(
            asbKlasifikasi.id,
            asbKlasifikasi,
        );
