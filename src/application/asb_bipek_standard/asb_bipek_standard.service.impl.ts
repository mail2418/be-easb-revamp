...
export class AsbBipekStandardServiceImpl extends AsbBipekStandardService {
    constructor(private readonly repository: AsbBipekStandardRepository) {
        super();
    }
    ...
    async update(dto: UpdateAsbBipekStandardDto): Promise<AsbBipekStandard> {
        ...
        if (!existing) {
            throw new NotFoundException(`AsbBipekStandard with id ${dto.id} not found`);
        }
    ...
    async delete(id: number): Promise<void> {
        ...
        if (!existing) {
            throw new NotFoundException(`AsbBipekStandard with id ${id} not found`);
        }
    ...
    async getByAsb(dto: GetAsbBipekStandardByAsbDto): Promise<{
        data: AsbBipekStandard[];
        total: number;
        page: number;
        amount: number;
        totalPages: number;
