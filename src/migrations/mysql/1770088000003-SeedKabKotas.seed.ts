...
export class SeedKabKotas1770088000003 implements MigrationInterface {
    name = 'SeedKabKotas1770088000003';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const kabkota of kabkotas) {
            // Get province ID by kode
            const provinces = await queryRunner.query(
                `SELECT id FROM \`provinces\` WHERE \`kode\` = ?`,
            ...
            await queryRunner.query(
                `INSERT IGNORE INTO \`kabkotas\` (\`kode\`, \`nama\`, \`province_id\`, \`is_active\`)
                 VALUES (?, ?, ?, ?)`,
