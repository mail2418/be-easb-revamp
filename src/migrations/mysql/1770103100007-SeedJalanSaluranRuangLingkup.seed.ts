...
export class SeedJalanSaluranRuangLingkup1770103100007 implements MigrationInterface {
    name = 'SeedJalanSaluranRuangLingkup1770103100007';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        for (const row of this.rows) {
            await queryRunner.query(
                `INSERT IGNORE INTO \`jalan_saluran_ruang_lingkup\` (\`id_jenis_usulan\`, \`deskripsi_ruang_lingkup\`) VALUES (?, ?)`,
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        for (const row of this.rows) {
            await queryRunner.query(
                `DELETE FROM \`jalan_saluran_ruang_lingkup\` WHERE \`id_jenis_usulan\` = ? AND \`deskripsi_ruang_lingkup\` = ?`,
                [row.id_jenis_usulan, row.deskripsi_ruang_lingkup],
            );
        }
    }
