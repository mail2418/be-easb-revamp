...
export class SeedAsbFungsiRuang1770088000005 implements MigrationInterface {
    name = 'SeedAsbFungsiRuang1770088000005';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const fungsiRuang of asbFungsiRuangs) {
            await queryRunner.query(
                `INSERT IGNORE INTO \`asb_fungsi_ruangs\` (\`nama_fungsi_ruang\`, \`koef\`, \`is_active\`)
                 VALUES (?, ?, ?)`,
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const placeholders = fungsiRuangList.map(() => '?').join(', ');
        await queryRunner.query(
